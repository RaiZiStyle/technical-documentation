...
#ifndef STM32_PERIPHERALS_STM32FLASH_H_
#define STM32_PERIPHERALS_STM32FLASH_H_
...
#include <assert.h>
#include <stdint.h>
// #include "system_h.h"  //Add system_h.h in project to include system libraries (ex : stm32h7xx.h)
#include "dbgConsol.h"
...
// DEFINES
#define FLASH_ALIGN_UP(value, align)   (((value) + ((align) - 1)) & ~((align) - 1))




#ifdef STM32G474xx
#define FLASH_SET_PAGE(STRUCT, ADDR) \
    ((STRUCT).Page = (((ADDR) < (FLASH_BASE + FLASH_BANK_SIZE)) ? \
    (((ADDR) - FLASH_BASE) / FLASH_PAGE_SIZE) : \
    (((ADDR) - (FLASH_BASE + FLASH_BANK_SIZE)) / FLASH_PAGE_SIZE)))

#define FLASH_SET_BANK(STRUCT, ADDR) \
    ((STRUCT).Banks = ((ADDR) <= (FLASH_BASE + (FLASH_SIZE / 2) - 1U) ? FLASH_BANK_1 : FLASH_BANK_2))


/*
 * Basically the "FLASH_EraseInitTypeDef" have a Page field and sometime a PageAddress. This flash librairie was tested with the 'PageAddress' field
 * I don't know the flash librairie will work with this simple fix used to compile t
 */
#elif defined(STM32F1)
#define FLASH_SET_PAGE(STRUCT, ADDR) ((STRUCT).PageAddress = (ADDR))
#define FLASH_SET_BANK(STRUCT, ADDR) \
    ((STRUCT).Banks = ((ADDR) <= (FLASH_BASE + (FLASH_SIZE / 2) - 1U) ? FLASH_BANK_1 : FLASH_BANK_2))
#warning "The structure FLASH_EraseInitTypeDef don't have the same field between micro. This macro wasn't tested with STM32F1 MCU"
#else
#warning "The structure FLASH_EraseInitTypeDef don't have the same field between micro. This macro wasn't tested we the new field"
// #error "Microcontrôleur non supporté"
#endif

#if defined(TYPEPROGRAM_WORD) && (TYPEPROGRAM_WORD + 0)
#define MACRO_HAS_VALUE 1
#else
#define TYPEPROGRAM_WORD 0
#warning "stm32Flash.{c/h} will probably not work ! This software is provided as is, without warranty of any kind."
/*
 * Some micro work with 32bit for the flash, some work with 64bit.
 * This library was written with a STM32F1.
 * The issue is that the `stm32Flash.{c/h}` use "TYPEPROGRAM_WORD", but in some micro this define is empty. And since it's empty the compilation doesn't work.
 */
#endif
...
#define STM32_FLASH_USER_START_ADDR (uint32_t)(ADDR_FLASH_PAGE_127)
#define STM32_FLASH_USER_END_ADDR (uint32_t)(STM32_FLASH_USER_START_ADDR  + FLASH_PAGE_SIZE)
////////////////////////////////////////
// TYPES DEFINITIONS
...
bool stm32Flash_concatenateStringAsUint32(uint32_t *uint32Buff, int uint32BuffSize,
                                          char *string2DBuff, int stringMaxLength, int nbString);
bool stm32Flash_splitStringFromUint32(uint32_t *uint32Buff, int uint32BuffSize,
                                      char *string2DBuff, int stringMaxLength, int nbString);
...
#endif /* STM32_PERIPHERALS_STM32FLASH_H_ */
